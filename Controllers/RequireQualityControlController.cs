        ...
        private async Task<bool> SendMail(RequireQualityControl HasData)
        {
            if (this.EmailClass.IsValidEmail(HasData.MailReply))
            {
                ...
                var BodyMessage = "<body style=font-size:11pt;font-family:Tahoma>" +
                                        ...
                                        $"เรียน คุณ{EmpName}" +
                                        $"<p>เรื่อง การเปิดคำขอตรวจคุณภาพงานเลขที่ {HasData.RequireQualityNo} </p>" +
                                        ...
                                        $"<p>\"คุณ{EmpName}\" " +
                                        $"สามารถเข้าไปตรวจติดตามข้อมูลได้ <a href='http://{Request.Host}/qualitycontrol/qualitycontrol/link-mail/{HasData.RequireQualityControlId}'>ที่นี้</a> </p>" +
            ...
            if (key > 0)
            {
                ...
                if (HasData != null)
                {
                    ...
                    var GenarateData = new RequireQualityControlViewModel()
                    {
                        ...
                        LocationQualityControlId = HasData.LocationQualityControlId,
            ...
            try
            {
                    ...
                    if (!string.IsNullOrEmpty(Schedule.Filter))
                    {
                        ...
                        foreach (var keyword in filters)
                        {
                            QueryData = QueryData.Where(x => x.Description.ToLower().Contains(keyword) ||
                                                             x.Remark.ToLower().Contains(keyword) ||
                                                             x.RequireQualityNo.ToLower().Contains(keyword) ||
                                                             x.LocationQualityControl.Name.ToLower().Contains(keyword) ||
                                                             x.WorkGroupQualityControl.Name.ToLower().Contains(keyword) ||
                                                             x.InspectionPoint.Name.ToLower().Contains(keyword) ||
                                                             x.WorkGroupQualityControl.Name.ToLower().Contains(keyword) ||
                ...
                if (GetData.Any())
                {
                    ...
                    foreach (DateTime day in EachDate.EachDate(MinDate.Value, MaxDate.Value))
                    {
                        ...
                        ColumnGroupBtm.Add(new DateTime(day.Year, day.Month, day.Day, 8, 0, 0), $"Col{countCol.ToString("00")}");
                        countCol++;
                        ColumnGroupBtm.Add(new DateTime(day.Year, day.Month, day.Day, 13, 0, 0), $"Col{countCol.ToString("00")}");
                        countCol++;
                    }
                    ...
                    foreach (var Data in GetData.OrderBy(x => x.RequireDate).ThenBy(x => x.CreateDate))
                    {
                        ...
                        rowData.Add("InspectionPoint", Data?.InspectionPoint?.Name ?? "Not Found");
                        rowData.Add("Progress", Progress);
                        rowData.Add("QualityControlStatus", Data?.QualityControlResult?.QualityControlStatus != null ? Data.QualityControlResult.QualityControlStatus : QualityControlStatus.Failed);
                        ...
                        {
                            if (ColumnGroupBtm.Any(x => x.Key.Date == Data.ResponseDate.Value.Date))
                            {
                                ...
                                rowData.Add("Response", ColumnGroupBtm.FirstOrDefault(x => x.Key == HasDate).Value);
                            }
                        }
                        // End new
                        ...
                        if (Data != null)
                        {
                            ...
                            {
                                if (ColumnGroupBtm.Any(x => x.Key.Date == Data.RequireDate.Date))
                                {
                                    ...
                                    // Add row data
                                    rowData.Add(ColumnGroupBtm.FirstOrDefault(x => x.Key == HasDate).Value, 1);
                                }
                            }
                            ...
                            {
                                if (ColumnGroupBtm.Any(x => x.Key.Date == Data?.QualityControlResult?.QualityControlResultDate.Value.Date))
                                {
                                    ...
                                    var Col = ColumnGroupBtm.FirstOrDefault(x => x.Key == HasDate);
                    ...
                    return new JsonResult(new
                    {
                        ...
                        ColumnsLow = ColumnGroupBtm.OrderBy(x => x.Key.Date).Select(x => x.Key.ToString("dd/HH:mm")),
                        ColumnsAll,
                        DataTable
                    }, this.DefaultJsonSettings);
                }
